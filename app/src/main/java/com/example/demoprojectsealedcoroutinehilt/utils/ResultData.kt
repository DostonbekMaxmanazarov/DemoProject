...
sealed class ResultData<T> {
    ...
    inline fun onResult(f: (Int?, String?, Throwable?, T?) -> Unit): ResultData<T> =
        onMessage { f(null, it, null, null) }.onResource { f(it, null, null, null) }
            .onFailure { f(null, null, it, null) }.onData { f(null, null, null, it) }
    ...
    companion object {
        fun <T> message(
            message: String,
            level: MessageData.Level = MessageData.Level.MAYBE_IGNORE
        ): ResultData<T> = Message(MessageData.message(message, level))
        ...
        fun <T> resource(
            @StringRes res: Int,
            level: MessageData.Level = MessageData.Level.MAYBE_IGNORE
        ): ResultData<T> = Message(MessageData.resource(res, level))

        fun <T> failure(
            exception: Throwable,
            level: MessageData.Level = MessageData.Level.MAYBE_IGNORE
        ): ResultData<T> = Message(MessageData.failure(exception, level))

        fun <T> data(
            data: T,
            level: MessageData.Level = MessageData.Level.MAYBE_IGNORE
        ): ResultData<T> = Data(data)
    }
