package com.example.demoprojectsealedcoroutinehilt.utils
...
sealed class MessageData(val level: Level) {
    ...
    internal class Resource internal constructor(@StringRes val res: Int, level: Level) :
        MessageData(level)

    internal class Message internal constructor(val message: String, level: Level) :
        MessageData(level)

    internal class Failure internal constructor(val exception: Throwable, level: Level) :
        MessageData(level)
    ...
    inline fun onResult(f: (Int) -> Unit): MessageData {
        if (isResource())
    ...
    companion object {
        fun message(message: String, level: Level = Level.MAYBE_IGNORE): MessageData =
            Message(message, level)

        fun resource(@StringRes res: Int, level: Level = Level.MAYBE_IGNORE): MessageData =
            Resource(res, level)

        fun failure(exception: Throwable, level: Level = Level.MAYBE_IGNORE): MessageData =
            Failure(exception, level)
    }
}